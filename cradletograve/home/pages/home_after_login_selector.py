...
if not os.path.isfile(image_path):
    ...
    with col1:
        st.link_button("일자리를 구하고 계신가요?","http://localhost:8507")
        st.markdown("")
        st.markdown("**AIHR**")
    ...
    with col1:
        st.link_button("두뇌개발","http://localhost:8502")
        st.markdown(":brain: :sunrise_over_mountains:")
    with col2:
        st.link_button("복지관","http://localhost:8503")
        st.markdown('''   :spider_web:    :computer:''')
    with col3:
        st.link_button("자료조사","http://localhost:8504")
        st.markdown('''   :school:    :eyes:''')
    with col4:
        st.link_button("데일리 리포트","http://localhost:8505")
        st.markdown('''   :newspaper:    :envelope_with_arrow:''')
    with col5:
        st.link_button("오늘의 운세","http://localhost:8506")
